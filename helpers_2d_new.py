...
def get_data_preprocessed(act_exv, act_dump, pln_exv, pln_dump, output_dir):
    ...
    return clean_act_exv, clean_act_dump, clean_pln_exv, clean_pln_dump
...
def split_planned_done_new(done_file, planned_file, output_dir, area_type):
    ...
    shp_folder = os.path.join(output_dir, "shape_file_outputs")
...
def plot_category_chart(output_dir, category, labels=None, colors=None, figsize=(4,4)):
    ...
    if not available_files:
        raise FileNotFoundError(f"No {category} shapefiles found in shape_file_outputs.")
...
#     ws = wb.active  # use first sheet, or specify ws = wb["SheetName"]

#     # === Find last used row to start appending ===
#     start_row = ws.max_row + 2  

#     # === Insert Excavation Table ===
#     ws.cell(row=start_row, column=1, value="Excavation Summary")
#     for r_idx, row in enumerate(dataframe_to_rows(df_excavation, index=False, header=True), start=start_row+1):
#         for c_idx, value in enumerate(row, start=1):
#             ws.cell(row=r_idx, column=c_idx, value=value)

#     # === Insert Excavation Chart ===
#     img_exc = XLImage(img_path_excavation)
#     img_exc.width, img_exc.height = 400, 400
#     ws.add_image(img_exc, f"E{start_row+1}")  # Place chart beside the table

#     # === Find next available row after Excavation Table ===
#     next_row = ws.max_row + 3  

#     # === Insert Dump Table ===
#     ws.cell(row=next_row, column=1, value="Dump Summary")
#     for r_idx, row in enumerate(dataframe_to_rows(df_dump, index=False, header=True), start=next_row+1):
#         for c_idx, value in enumerate(row, start=1):
#             ws.cell(row=r_idx, column=c_idx, value=value)

#     # === Insert Dump Chart ===
#     img_dump = XLImage(img_path_dump)
#     img_dump.width, img_dump.height = 400, 400
#     ws.add_image(img_dump, f"E{next_row+1}")  # Place chart beside the table
